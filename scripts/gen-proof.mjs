...
const B = "build";
const rnd = () => BigInt("0x" + [...crypto.getRandomValues(new Uint8Array(28))]
...
await snarkjs.wtns.calculate(
  ...
  `${B}/passport_witness_js/passport_witness.wasm`, `${B}/pw.wtns`,
);
...
const { proof, publicSignals } = await snarkjs.groth16.fullProve(
  input, `${B}/agent_passport_js/agent_passport.wasm`, `${B}/agent_passport_final.zkey`,
);
...
writeFileSync(`${B}/arg_public.json`, JSON.stringify(publicSignals.map(String)));

console.log("publicSignals [registryRoot, nullifierHash, agentId, spendCap]:");
